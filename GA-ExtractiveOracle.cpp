...
string chapter_sentences[7000], reference_summary;
...
double rouge1_fscore(string doc_A, string doc_B){
    vector<string> doc_A_splitted = split_string(doc_A);
    vector<string> doc_B_splitted = split_string(doc_B);
    map<string, int> one_gram_A, one_gram_B;
    for(int i = 0; i < (int)doc_A_splitted.size(); i++){
        if(one_gram_A.find(doc_A_splitted[i]) == one_gram_A.end()) one_gram_A[doc_A_splitted[i]] = 0;
        one_gram_A[doc_A_splitted[i]]++;
    }
    for(int i = 0; i < (int)doc_B_splitted.size(); i++){
        if(one_gram_B.find(doc_B_splitted[i]) == one_gram_B.end()) one_gram_B[doc_B_splitted[i]] = 0;
        one_gram_B[doc_B_splitted[i]]++;
    }
    double P = 0, R = 0;
    int match_cnt = 0, word_cnt = 0;
    for(map<string, int> :: iterator it = one_gram_A.begin(); it != one_gram_A.end(); it++){
        word_cnt += (*it).se;
        if(one_gram_B.find((*it).fi) != one_gram_B.end()){
            match_cnt += min((*it).se, one_gram_B[(*it).fi]);
        }
    }
    if(word_cnt != 0) P = (double)match_cnt / (double)word_cnt;
    match_cnt = 0; word_cnt = 0;
    for(map<string, int> :: iterator it = one_gram_B.begin(); it != one_gram_B.end(); it++){
        word_cnt += (*it).se;
        if(one_gram_A.find((*it).fi) != one_gram_A.end()){
            match_cnt += min((*it).se, one_gram_A[(*it).fi]);
        }
    }
    if(word_cnt != 0) R = (double)match_cnt / (double)word_cnt;
    if(P + R < EPS) return 0;
    else return (2.0 * P * R) / (P + R);
}

double rouge2_fscore(string doc_A, string doc_B){
    vector<string> doc_A_splitted = split_string(doc_A);
    vector<string> doc_B_splitted = split_string(doc_B);
    map<string, int> two_gram_A, two_gram_B;
    for(int i = 0; i < (int)doc_A_splitted.size() - 1; i++){
        string w = doc_A_splitted[i] + " " + doc_A_splitted[i + 1];
        if(two_gram_A.find(w) == two_gram_A.end()) two_gram_A[w] = 0;
        two_gram_A[w]++;
    }
    for(int i = 0; i < (int)doc_B_splitted.size() - 1; i++){
        string w = doc_B_splitted[i] + " " + doc_B_splitted[i + 1];
        if(two_gram_B.find(w) == two_gram_B.end()) two_gram_B[w] = 0;
        two_gram_B[w]++;
    }
    double P = 0, R = 0;
    int match_cnt = 0, two_gram_cnt = 0;
    for(map<string, int> :: iterator it = two_gram_A.begin(); it != two_gram_A.end(); it++){
        two_gram_cnt += (*it).se;
        if(two_gram_B.find((*it).fi) != two_gram_B.end()){
            match_cnt += min((*it).se, two_gram_B[(*it).fi]);
        }
    }
    if(two_gram_cnt != 0) P = (double)match_cnt / (double)two_gram_cnt;
    match_cnt = 0; two_gram_cnt = 0;
    for(map<string, int> :: iterator it = two_gram_B.begin(); it != two_gram_B.end(); it++){
        two_gram_cnt += (*it).se;
        if(two_gram_A.find((*it).fi) != two_gram_A.end()){
            match_cnt += min((*it).se, two_gram_A[(*it).fi]);
        }
    }
    if(two_gram_cnt != 0) R = (double)match_cnt / (double)two_gram_cnt;
    if(P + R < EPS) return 0;
    else return (2.0 * P * R) / (P + R);
}

short int dp[20000][20000];

double rougeL_fscore(string doc_A, string doc_B){
    vector<string> doc_A_splitted = split_string(doc_A);
    vector<string> doc_B_splitted = split_string(doc_B);
    if(doc_A_splitted.empty() || doc_B_splitted.empty()) return 0;
    for(int i = 0; i <= (int)doc_A_splitted.size(); i++){
        for(int j = 0; j <= (int)doc_B_splitted.size(); j++){
            if(i == 0 || j == 0) dp[i][j] = 0;
            else if(doc_A_splitted[i - 1] == doc_B_splitted[j - 1]) dp[i][j] = dp[i - 1][j - 1] + 1;
            else dp[i][j] = max(dp[i][j - 1], dp[i - 1][j]);
        }
    }
    double P = (double)dp[(int)doc_A_splitted.size()][(int)doc_B_splitted.size()] / (double)doc_A_splitted.size();
    double R = (double)dp[(int)doc_A_splitted.size()][(int)doc_B_splitted.size()] / (double)doc_B_splitted.size();
    if(P + R < EPS) return 0;
    else return (2.0 * P * R) / (P + R);
}

double calculate_profit(vector<int> sentences_id){
    if(sentences_id.empty()) return 0;
    string generated_summary = "";
    sort(sentences_id.begin(), sentences_id.end());
    for(int i = 0; i < (int)sentences_id.size(); i++){
        if(i > 0) generated_summary += " ";
        generated_summary += chapter_sentences[sentences_id[i]];
    }
    return (rouge1_fscore(generated_summary, reference_summary) + rouge2_fscore(generated_summary, reference_summary) + rougeL_fscore(generated_summary, reference_summary)) / 3.0;
}
...
int main(){
    ...
    ifstream inputFile("GA-ExtractiveOracle-input.txt");
    ofstream outputFile("GA-ExtractiveOracle-output.txt");
    
    inputFile >> num_sentences >> summary_length_limit;
    for(int i = 0; i < num_sentences; i++) inputFile >> sentences_length[i];
    getline(inputFile, reference_summary); getline(inputFile, reference_summary);
    for(int i = 0; i < num_sentences; i++) getline(inputFile, chapter_sentences[i]);
    ...
    outputFile << best_solution.profit * 3.0 << '\n';
