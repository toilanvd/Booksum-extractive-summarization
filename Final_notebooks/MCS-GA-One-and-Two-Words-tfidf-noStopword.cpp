...
void build_document_frequency(){
    ifstream vocabulary_file("vocabulary_cnn-dailymail.txt");
...
bool compare_solutions(solution sol_A, solution sol_B){ // Return true if solution A is better than solution B
    ...
    else if(sol_A.profit + EPS < sol_B.profit) return false;
    else if(sol_A.cost + EPS < sol_B.cost) return true;
    else return false;
}
...
double calculate_profit(vector<int> sentences_id){
    ...
    for(int i = 0; i < (int)sentences_id.size(); i++){
        ...
        for(int j = 0; j < (int)chapter_sentences_words[id].size(); j++){
            ...
            int freq = (doc_freq.find(one_word) == doc_freq.end()) ? 1 : doc_freq[one_word];
            if(checked.find(one_word) == checked.end()) profit += (double)(string_cnt[one_word] - 1) * log2((double)num_of_vocab_doc / (double)freq);
            ...
            freq = (doc_freq.find(two_words) == doc_freq.end()) ? 1 : doc_freq[two_words];
            if(checked.find(two_words) == checked.end()) profit += (double)((string_cnt[two_words] - 1) * 2) * log2((double)num_of_vocab_doc / (double)freq);
...
int main(){
    ...
    ifstream inputFile("MCS-GA-One-and-Two-Words-tfidf-noStopword-input.txt");
    ofstream outputFile("MCS-GA-One-and-Two-Words-tfidf-noStopword-output.txt");
    ...
    while(best_solution_not_changed_cnt < G_BEST || sum_profit_not_increase_cnt < G_SUM_PROFIT){
        ...
        for(int i = 0; i < POPULATION_SIZE; i++){
            // Crossover
            double x = (double)(rand() % 10001) / 10000.0 * (double)sum_len[POPULATION_SIZE - 1];
