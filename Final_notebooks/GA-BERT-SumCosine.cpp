...
int num_sentences, summary_length_limit, sentence_vector_length;
int sentences_length[7000];
vector<double> sentences_embedding[7000], chapter_embedding;
string chapter_sentences[7000];
...
vector<double> sum_embedding_vectors(vector<int> sentences_id){
    vector<double> res(sentence_vector_length, 0);
    for(int i = 0; i < (int)sentences_id.size(); i++){
        for(int j = 0; j < sentence_vector_length; j++){
            res[j] += sentences_embedding[sentences_id[i]][j];
        }
    }
    return res;
}

double cosine_similarity(vector<double> vec_A, vector<double> vec_B){
    double sum_AB = 0, sum_A = 0, sum_B = 0;
    for(int i = 0; i < sentence_vector_length; i++){
        sum_AB += vec_A[i] * vec_B[i];
        sum_A += vec_A[i] * vec_A[i];
        sum_B += vec_B[i] * vec_B[i];
    }
    return sum_AB / sqrt(sum_A * sum_B);
}

double calculate_profit(vector<int> sentences_id){
    vector<double> summary_embedding = sum_embedding_vectors(sentences_id);
    return cosine_similarity(summary_embedding, chapter_embedding);
}
...
int main(){
    ...
    ifstream inputFile("GA-BERT-SumCosine-input.txt");
    ofstream outputFile("GA-BERT-SumCosine-output.txt");
    
    inputFile >> num_sentences >> summary_length_limit >> sentence_vector_length;
    for(int i = 0; i < num_sentences; i++) inputFile >> sentences_length[i];
    for(int i = 0; i < num_sentences; i++){
        for(int j = 0; j < sentence_vector_length; j++){
            double x;
            inputFile >> x;
            sentences_embedding[i].push_back(x);
        }
        getline(inputFile, chapter_sentences[i]);
        if(chapter_sentences[i][0] == ' ') chapter_sentences[i].erase(chapter_sentences[i].begin());
    }
    vector<int> temp_vector;
    for(int i = 0; i < num_sentences; i++) temp_vector.push_back(i);
    chapter_embedding = sum_embedding_vectors(temp_vector);
    ...
    while(best_solution_not_changed_cnt < G_BEST || sum_profit_not_increase_cnt < G_SUM_PROFIT){
        ...
        for(int i = 0; i < POPULATION_SIZE; i++){
            ...
            // Mutation
            // Remove a sentence
            if((int)population[i].sentences.size() > 1){
